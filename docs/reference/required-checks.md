...
| `ci / test` | `uv run pytest` passes on `.python-version`, and on each version in the `extra-python-versions` input | A failing test, or one that fails only on an extra version (the log's `python <version>` line says which) | Run the same `pytest` locally, with `uv run --python <version>` for an extra version |
...
| `ci / floor-check` | The repository still has the floor `/plinth:floor-check` reads: the document set, agent settings, the project files its archetype needs, the labels, and a ruleset and CodeQL setup that still require these checks | At least one FAIL line in the job summary | Run `/plinth:floor-check`: each FAIL comes with one fix |
