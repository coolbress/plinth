...
and the issue and pull-request templates that actually apply to it. "Actually
apply" includes the owner's `owner/.github` defaults, which GitHub uses when the
repository has none of its own — a local `.github/ISSUE_TEMPLATE` holding any
template or a `config.yml` replaces the shared set entirely, per folder, never
per file.
...
**In a consumer repository, its conventions win.** Everything below is what
*this* repository does and what plinth offers where a repository has nothing.
Use their forms, their fields, their labels. Never rewrite an existing
repository's templates into plinth's shape.

## Three kinds of issue (this repository's default)
...
| A record: research, a decision, a map, a spec | blank issue; the kind is a label: `wayfinder:map`, `wayfinder:research`, `wayfinder:grilling`, or `spec` (the door creates them), plus the triage label while open | `## Question`, `## Findings`, `## Decision`, `## Used by` for research; the map and the spec keep their own shape |
...
- **An issue**: the problem, why it is worth solving, what done looks like,
  the decisions taken and their reasons, and the outcome on closing.
...
Issues and pull requests are public, and they record the product, not the
owner's computer. An environment detail goes in when a result depends on it or
it limits what was observed, written as the condition without the person: "a
checkout path containing a space", "git 2.36", "with the sandbox on", "on one
machine", "not tried on an open pull request". Home paths, account and folder
names and credential values stay out, or are replaced by what matters about
them. Local housekeeping the result does not depend on stays out too: what was
installed, registered or removed on the owner's computer, and which sessions
ran. Keep decisions, dates, numbers, links, acceptance criteria and checkbox
states. A project term such as the everyday token is not personal and stays. An
...
- **Resolve**: put the answer in the body — the shape the table above gives that child's kind, and the body is the record, a comment is not. A `research`, `prototype` or `grilling` child is a record: `## Findings` and `## Decision`, and `## Decision` is its ending, so it does not also take an `## Outcome`. A `task` child is a task like any other and ends in `## Outcome`. Then `gh issue close <n>`, then one line (gist + link) under the map's Decisions-so-far.
