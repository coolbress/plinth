...
- **Resolve**: put the answer in the body (`## Findings` and `## Decision`, the shape the table above gives a record; the body is the record, a comment is not), then `gh issue close <n>`, then one line (gist + link) under the map's Decisions-so-far. `## Decision` is a record's ending, so it does not also take the `## Outcome` a task or a bug report carries.
