...
| `floor-check [owner/name]` | Reads an existing repository against the same floor the CI job checks, the live ruleset, whether it is behind the template plinth is tested with today, and this machine's sandbox; lists each miss with one fix; changes nothing | You or the agent |
...
[plinth-template v1.5.0](https://github.com/coolbress/plinth-template/releases/tag/v1.5.0),
